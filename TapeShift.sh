...
function trim_output() {
    ...
    if [[ "$user_choice" =~ ^[yY]$ ]]; then
        ...
        echo ""
        echo "----------------------------------------------------------------"
        echo -e "${ANSI_GREY}ffmpeg -copyts -ss ${start_param} -i \"${output_path%.*}.mp4\" -to ${end_param} -map 0 -c copy \"${output_path%.*}_TRIMMED.mp4\" 2>>\"${output_path%.*}.log\"${ANSI_CLEAR}"
        echo "----------------------------------------------------------------"
        echo ""
        ...
        if [[ "$user_choice" =~ ^[yY]$ ]]; then
            ...
            # Write to log.
            {
                ...
                echo "----------------     FFMPEG COMMAND     ----------------"
                echo "ffmpeg -copyts -ss ${start_param} -i \"${output_path%.*}.mp4\" -to ${end_param} -map 0 -c copy \"${output_path%.*}_TRIMMED.mp4\" 2>>\"${output_path%.*}.log\""
                echo ""
                echo "---------------- FFMPEG OUTPUT (STDERR) ----------------"
            } >> "${output_path%.*}.log"
